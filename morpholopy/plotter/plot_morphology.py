...
def output_accumulative_densities(galaxy_data, siminfo):
    ...
    np.savetxt(f"{siminfo.output_path}/accumulative_surface_density_gas_grid"+siminfo.name+".txt",
               np.transpose([galaxy_data.surface_density[1:], galaxy_data.SFR_density[1:],
                             galaxy_data.metallicity[1:]]))

    np.savetxt(f"{siminfo.output_path}/accumulative_surface_density_ratios_grid"+siminfo.name+".txt",
               np.transpose([galaxy_data.surface_density[1:], galaxy_data.ratio_densities[1:],
                             galaxy_data.metallicity[1:]]))

    np.savetxt(f"{siminfo.output_path}/accumulative_surface_density_ratios_radii"+siminfo.name+".txt",
               np.transpose([galaxy_data.radii_surface_density[1:], galaxy_data.radii_surface_ratio[1:]]))
...
def output_morphology(galaxy_data,siminfo):
    ...
    output_kappa(10**galaxy_data.stellar_mass,galaxy_data.gas_kappa_co,0,siminfo)
    ...
    output_axis_ratios(10**galaxy_data.stellar_mass,axis_ratios,4,siminfo)
    ...
    output_axis_ratios(10**galaxy_data.stellar_mass,axis_ratios,0,siminfo)

    # plot surface densities
    output_accumulative_densities(galaxy_data, siminfo)
...
def plot_morphology(siminfo, name_list):

    plot_momentum(siminfo, name_list)
    plot_kappa(siminfo, name_list)
    plot_axis_ratios(siminfo,name_list)

def plot_momentum(siminfo,name_list):
    ...
    for parttype in parttype_list:
        ...
        plt.savefig(f"{siminfo.output_path}/momentum_parttype_%i.png"%parttype, dpi=200)
        plt.close()


def plot_kappa(siminfo,name_list):
    ...
    for parttype in parttype_list:
        ...
        ax.tick_params(direction='in', axis='both', which='both', pad=4.5)
        plt.legend(labelspacing=0.2, handlelength=2, handletextpad=0.4, frameon=False)
        plt.savefig(f"{siminfo.output_path}/Kappa_co_parttype_%i.png" % parttype, dpi=200)
        plt.close()


def plot_axis_ratios(siminfo,name_list):
    ...
    for parttype in parttype_list:
        ...
        ax.tick_params(direction='in', axis='both', which='both', pad=4.5)
        plt.legend(labelspacing=0.2, handlelength=2, handletextpad=0.4, frameon=False)
        plt.savefig(f"{siminfo.output_path}/Axis_ratios_parttype_%i.png" % parttype, dpi=200)
        plt.close()
