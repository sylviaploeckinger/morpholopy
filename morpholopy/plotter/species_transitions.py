...
from plotter.helpers import get_Schruba_data, get_Schruba_upperlimits, get_Bigiel2008_data
...
def species_transitions_combined(sim_name, output_path, halo_min_stellar_mass):
        ...
        redshift, binsize, SigmaHIH2, SigmaH2_over_HI, twelveplusOH, Sigma_star = np.genfromtxt(filename_text_f1, comments='#', unpack = True, usecols = (2,4,5,6,7,8))

        text =  sim_name + ', z = %.1f'%(redshift[0]) + '\n'
        text += "Limited to galaxies with:\n"
        text += r"log$_{\mathrm{10}}$ M$_{\star}$ [M$_{\odot}$] > %.2f"%(np.log10(halo_min_stellar_mass))
        ...
        for irad in range(len(radialbinsizes)):
                ...
                sc = ax.scatter(x, y, c = c, vmin = twelve_plus_logOH_min, vmax = twelve_plus_logOH_max, cmap = cmRdBu, edgecolors = 'black', label = 'Azim.avg. %.2f kpc'%(radialbin_kpc), marker = markers[irad])
        ...
        fig.savefig(output_path + '/Species_transition_'+sim_name + '_Schruba2011.png', dpi = 150)
        plt.close()
        ...
        for irad in range(len(radialbinsizes)):
                ...
                sc = ax.scatter(x, y, c = c, vmin = logstarmin, vmax = logstarmax, cmap = cmRdBu, edgecolors = 'black', label = 'Azim.avg. %.2f kpc'%(radialbin_kpc), marker = markers[irad])
        ...
        fig.savefig(output_path + '/Species_transition_'+sim_name + '_Schruba2011_Sigma_star.png', dpi = 150)
        plt.close()
        ...
        redshift_fp, binsize_fp, log10sfr_fp, log10H2_fp, log10HI_fp, log10star_fp = np.genfromtxt(filename_text_f2, comments='#', unpack = True, usecols = (2,4,5,6,7,8))
        ...
        hist, xe, ye = np.histogram2d(x, y, bins = nbins_hist, range = [[logstarmin, logstarmax],[ymin, ymax]])
        ...
        hist[hist <= np.log10(2.)] = np.nan


        ax.scatter(x, y, facecolors = 'none', edgecolors = 'grey', zorder = 0, s = 4)
        ax.imshow(hist, interpolation = 'none', origin = 'lower', extent = [xe[0], xe[-1], ye[0], ye[-1]], vmin = 0., zorder = 1, aspect = (logstarmax-logstarmin)/(ymax - ymin), cmap = 'Greys')

        ax.plot(logstarbins, -10.10 + 0.02 * logstarbins, color = '#CC0066', linestyle = 'dashed', zorder = 10)
        ax.plot(logstarbins[logstarbins>1.2], -10.10 + 0.02 * logstarbins[logstarbins>1.2], color = '#CC0066', linestyle = 'solid', zorder = 10, label = 'Sanchez+2021(EDGE fit)')
        ...
        hist, xe, ye = np.histogram2d(x, y, bins = nbins_hist, range = [[logmolmin, logmolmax],[ymin, ymax]])
        hist_resampled = scipy.ndimage.zoom(hist, 3)
        hist = np.log10(hist.T)
        hist_resampled = np.log10(hist_resampled.T)
        xe_resampled = np.linspace(xe[0], xe[-1], hist_resampled.shape[0], endpoint = True)
        ...
        ax.scatter(x, y, facecolors = 'none', edgecolors = 'grey', zorder = 0, s = 4)
        ax.imshow(hist, interpolation = 'none', origin = 'lower', extent = [xe[0], xe[-1], ye[0], ye[-1]], vmin = 0., zorder = 1, aspect = (logmolmax-logmolmin)/(ymax - ymin), cmap = 'Greys')

        ax.plot(logmolbins, -9.01 + (0.98-1.0) * logmolbins, color = '#CC0066', linestyle = 'dashed', zorder = 10)
        ax.plot(logmolbins[logmolbins>0.3], -9.01 + (0.98-1.0) * logmolbins[logmolbins>0.3], color = '#CC0066', linestyle = 'solid', zorder = 10, label = 'Sanchez+2021(EDGE fit)')
        ...
        ax.plot(logstarbins[logstarbins>1.2], -0.91 + (0.93-1.) * logstarbins[logstarbins>1.2], color = '#CC0066', linestyle = 'solid', label = 'Sanchez+2021(EDGE fit)', zorder = 10)
        ax.plot(logstarbins, -0.91 + (0.93-1.) * logstarbins, color = '#CC0066', linestyle = 'dashed', zorder = 10)
