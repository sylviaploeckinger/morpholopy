...
from plotter.html import make_web, add_metadata_to_web, \
    PlotsInPipeline, render_population_web, add_web_section
...
def plot_population_data(output_path, name_list):
    ...
    for name in name_list:
        ...
        plt.plot(mass_bins[minimum_range], fraction_passive[minimum_range], color=color[i], label=name)
    ...
    for name in name_list:
        ...
        minimum_range = histogram_all > 2
        plt.plot(mass_bins[minimum_range], fraction_active[minimum_range], color=color[i], label=name)
